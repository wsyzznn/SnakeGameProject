...
public class FoodTypeSupport extends TypeSupport {
    private String type_name = "Food";
    private static TypeCodeImpl s_typeCode = null;
    private static FoodTypeSupport m_instance = new FoodTypeSupport();
    ...
    public int copy_sampleI(Object dst,Object src) {
        Food FoodDst = (Food)dst;
        Food FoodSrc = (Food)src;
        FoodDst.copy(FoodSrc);
        return 1;
    }
    ...
    public int print_sample(Object _sample) {
        ...
        Food sample = (Food)_sample;
        System.out.println("sample.food_id:" + sample.food_id);
    ...
    public DataReader create_data_reader() {return new FoodDataReader();}

    public DataWriter create_data_writer() {return new FoodDataWriter();}
    ...
    public int serializeI(Object _sample ,long cdr) {
        ...
        if (!CDRSerializer.put_int(cdr, sample.food_id)){
            System.out.println("serialize sample.food_id failed.");
            return -2;
        }
    ...
    synchronized public int deserializeI(Object _sample, long cdr){
        ...
        if (!CDRDeserializer.get_int_array(cdr, tmp_int_obj, 1)){
            System.out.println("deserialize sample.food_id failed.");
            return -2;
        }
        sample.food_id= tmp_int_obj[0];
    ...
    public TypeCode get_typecode(){
        ...
        s_typeCode = factory.create_struct_TC("SnakeGame.Food");
        if (s_typeCode == null){
            System.out.println("create struct Food typecode failed.");
            return s_typeCode;
        }
        int ret = 0;
        ...
        if (memberTc == null){
            System.out.println("Get Member food_id TypeCode failed.");
